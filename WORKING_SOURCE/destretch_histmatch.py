...
vbifold = '/Volumes/VBI_External/pid_2_11/'
vbiexp ='BDJKM'
file = '/VBI_2024_08_11T20_35_54_333333_00656282_I_BDJKM_L1.fits'
...
fits.writeto(savfold+'pre-destretch_dataCube.fits',dataCube,overwrite=True)

#restore datacube
dataCube=fits.open(savfold+'pre-destretch_dataCube.fits')[0].data

#make a movie 
utilvbi.storeSequence(dataCube,savfold+'pre-destretch_'+filt+'.mp4', dpi=300, write=True)

Video(savfold+'re-destretch_'+filt+'.mp4', embed=True, width=600, height=600,writer='pillow')
...
fits.writeto(savfold+'postdestretch_dataCube.fits',dataCubeTracked,overwrite=True)

#restore tracked datacube
dataCubeTracked=fits.open(savfold+'postdestretch_dataCube.fits')[0].data

utilvbi.storeSequence(dataCubeTracked,savfold+'postdestretch_'+filt+'.mp4', dpi=300, write=True)
...
fits.writeto(savfold+'postdestretch_histomatch_dataCube.fits',dataCubeTrackedhist,overwrite=True)#save movie
utilvbi.storeSequence(dataCubeTrackedhist,savfold+'postdestretch_histomatch_'+filt+'.mp4', dpi=300, write=True)

Video(savfold+'postdestretch_histomatch_'+filt+'.mp4', embed=True, width=600, height=600)
