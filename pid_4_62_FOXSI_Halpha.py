...
path = '/Volumes/ViSP_External/FOXSI_5_Flare/'
path2 = '/Volumes/ViSP_External/FOXSI_5_Flare/'
folder1 = 'QWULJQ'
folder2 = ''  #AORVR is the QS at disk center for 11 August obs
...
wl = 656.415 # central wavelength, Ca II H
...
endstep=400#where does interesting bit end? 
...
## playing around with weighted mean to measure redshift in H-alpha, see if there's anything interesting
# weightedmean = np.zeros([np.shape(image_data_arr_arr)[0],np.shape(image_data_arr_arr)[2]])
# for i in range(np.shape(image_data_arr_arr)[0]):
#     for j in range(np.shape(image_data_arr_arr)[2]):
#         mask = ~np.isnan(image_data_arr_arr[i,:,j])
#         data = np.arange(np.shape(image_data_arr_arr)[1])

#         valid_data = data[mask]
#         valid_weights = image_data_arr_arr[i,mask,j]-image_data_arr_arr[30,mask,200]
        
#         mask2 = ~np.isnan(valid_weights)
        
#         valid_data2 = valid_data[mask2]
#         valid_weights2 = valid_weights[mask2]
        
#         if len(valid_data)>0 and i>30:
#             weightedmean[i,j]=np.average(valid_data2,weights=valid_weights2)
